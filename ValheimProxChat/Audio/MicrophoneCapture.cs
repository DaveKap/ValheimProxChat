...
namespace ValheimProxChat.Audio
{
    ...
    public class MicrophoneCapture : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            float boost = Configuration.MicrophoneBoost.Value;
            ...
            if (Configuration.PushToTalk.Value)
            {
                KeyCode pttKey = ParseKeyCode(Configuration.PushToTalkKey.Value);
                shouldTransmit = Input.GetKey(pttKey);
            }
            else if (Configuration.VoiceActivation.Value)
            {
                shouldTransmit = level > Configuration.VoiceActivationThreshold.Value;
            }
            else
            {
                // Always on
